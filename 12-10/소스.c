...
void searchByName(FILE* f, const char* name);
...
int main()
{
	...
	while (1)
	{
		...
		puts("학생 번호 검색 : 1, 학생 이름 검색 : 2, 학생 추가 : 3, 종료 : 0 입력 >>");
		scanf(" %d", &command);
...
void searchByName(FILE* f, const char* name)
{
	...
	fseek(f, 0, SEEK_SET);			// 파일의 처음 포인터에서 학생 번호만큼 뒤로 이동
	...
	while (!feof(f))
	{
		...
		fread(&score, sizeof(pscore), 1, f);			// 학생을 읽음
	}
}
